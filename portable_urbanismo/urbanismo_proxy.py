...
VERSIÓN PORTABLE / AUTOCONTENIDA
--------------------------------
Copia independiente del módulo `cpq.analysis.urbanismo_proxy`, SIN
dependencias del paquete CPQ. Pensada para integrarse en otro proyecto
copiando únicamente este archivo. La lógica de cálculo es idéntica al
original; lo único que cambia es que las 4 constantes por defecto (antes
`cpq.config.CFG`) están ahora definidas localmente más abajo.
...
Contrato de entrada (compute_urbanismo_proxy):
    - target_parcel_gdf : GeoDataFrame de 1 fila (parcela objetivo)
    - parcels_gdf       : GeoDataFrame con las parcelas del entorno
    - buildings_gdf     : GeoDataFrame con las huellas de edificios
    - roads_gdf         : GeoDataFrame de viales (opcional; mejora el frente)
    Todos deben estar en un CRS PROYECTADO en metros (p.ej. EPSG:25830).

Salida: ProxyResult con retranqueos, ocupación, edificabilidad, envolvente
edificable, estadísticos y score de confianza (flag use_proxy True/False).
...
Los valores por defecto (fallback cuando no hay datos/confianza) se definen
abajo en la clase `CFG`; edítalos para ajustarlos a tu normativa base.
"""
...
# ------------------------------------------------------------------------------
# Valores por defecto (antes provenían de cpq.config.CFG).
# Solo se usan como FALLBACK: cuando no hay datos suficientes o la
# mediana/percentil resulta NaN. Edita estos cuatro números para adaptarlos
# a tu normativa base.
# ------------------------------------------------------------------------------
@dataclass(frozen=True)
class _UrbanDefaults:
    OCUPACION_PORCENTAJE: float = 30.0       # % ocupación por defecto
    EDIFICABILIDAD_M2T_M2S: float = 0.4      # m² techo / m² suelo por defecto
    RETRANQUEO_FRONTAL_M: float = 5.0        # retranqueo a vial por defecto (m)
    RETRANQUEO_LATERAL_M: float = 3.0        # retranqueo lateral/fondo por defecto (m)
